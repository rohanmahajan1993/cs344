...
{
  // The reason that we can't use just one block is because the number of numRows * numCols is greater than 512
  // So block size will basically by default 22 * 22, which will be less than 512 threads
  int threadLimit = 512;
